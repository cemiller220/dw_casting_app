...
<script>
    ...
    export default {
        ...
        props: ['dancer', 'index', 'current_cast'],
        computed: {
            ...
            dancerStatusClass() {
                if (this.current_cast.cast.filter(dancer => dancer.name === this.dancer).length === 0) {
                    return '';
                } else {
                    return this.current_cast.cast.filter(dancer => dancer.name === this.dancer)[0].status === 'cast' ? 'list-group-item-success' : 'list-group-item-secondary';
                }
